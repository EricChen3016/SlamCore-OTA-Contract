...
- Align Phase 4 error correlation UUID acceptance with the existing attempt header: preserve valid lowercase, uppercase and mixed-case spelling exactly, retaining generated diagnostic UUIDs for missing/malformed headers. Keep operationCorrelationId canonical lowercase and legacy 2.0 surfaces unchanged; add examples and exact-echo/invalid-UUID regressions under the recorded Issue #5 architecture decision.
